...
const char *const PCJacobiTypes[]    = {"DIAGONAL","ROWMAX","ROWSUM","ROWSUMABS","PCJacobiType","PC_JACOBI_",0};
...
static PetscErrorCode  PCJacobiGetType_Jacobi(PC pc,PCJacobiType *type)
{
  ...
  } else if (j->userowsumabs) {
    *type = PC_JACOBI_ROWSUMABS;
  } else {
...
static PetscErrorCode PCSetUp_Jacobi(PC pc)
{
  ...
  */
  ...
  if (diag) {
    ...
    } else if (jac->userowsumabs) {
      ierr = MatGetRowSumAbs(pc->pmat,diag);CHKERRQ(ierr);
    } else {
  ...
  if (diagsqrt) {
    if (jac->userowmax) {
      ...
      ierr = MatGetRowSumAbs(pc->pmat,diagsqrt);CHKERRQ(ierr);
    } else {
...
/*@
   ...
   Input Parameters:
+  pc - the preconditioner context
-  type - PC_JACOBI_DIAGONAL, PC_JACOBI_ROWMAX, PC_JACOBI_ROWSUM, PC_JACOBI_ROWSUMABS

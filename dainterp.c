...
PetscErrorCode DMCreateInterpolation_DA_2D_Q0(DM dac,DM daf,Mat *A)
{
  ...
  ierr = DMDAGetInfo(dac,0,&Mx,&My,0,0,0,0,0,0,&bx,&by,0,0);CHKERRQ(ierr);
  ierr = DMDAGetInfo(daf,0,&mx,&my,0,0,0,0,&dof,0,0,0,0,0);CHKERRQ(ierr);
  if (bx == DM_BOUNDARY_PERIODIC) SETERRQ(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_WRONG,"Cannot handle periodic grid in x");
  if (by == DM_BOUNDARY_PERIODIC) SETERRQ(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_WRONG,"Cannot handle periodic grid in y");
  ratioi = mx/Mx;
  ratioj = my/My;
...
PetscErrorCode DMCreateInterpolation_DA_3D_Q0(DM dac,DM daf,Mat *A)
{
  ...
  if (bx == DM_BOUNDARY_PERIODIC) SETERRQ(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_WRONG,"Cannot handle periodic grid in x");
  if (by == DM_BOUNDARY_PERIODIC) SETERRQ(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_WRONG,"Cannot handle periodic grid in y");
  if (bz == DM_BOUNDARY_PERIODIC) SETERRQ(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_WRONG,"Cannot handle periodic grid in z");
  ratioi = mx/Mx;
  ratioj = my/My;
  ratiol = mz/Mz;
  if (ratioi*Mx != mx) SETERRQ(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_WRONG,"Fine grid points must be multiple of coarse grid points in x");
...
PetscErrorCode  DMCreateInterpolation_DA(DM dac,DM daf,Mat *A,Vec *scale)
{
  ...
  if (dimc != dimf) SETERRQ2(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_INCOMP,"Dimensions of DMDA do not match %D %D",dimc,dimf);CHKERRQ(ierr);
  if (dofc != doff) SETERRQ2(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_INCOMP,"DOF of DMDA do not match %D %D",dofc,doff);CHKERRQ(ierr);
  if (sc != sf) SETERRQ2(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_INCOMP,"Stencil width of DMDA do not match %D %D",sc,sf);CHKERRQ(ierr);
  if (bxc != bxf || byc != byf || bzc != bzf) SETERRQ(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_INCOMP,"Boundary type different in two DMDAs");CHKERRQ(ierr);
  if (stc != stf) SETERRQ(PetscObjectComm((PetscObject)daf),PETSC_ERR_ARG_INCOMP,"Stencil type different in two DMDAs");CHKERRQ(ierr);
...
PetscErrorCode  DMCreateAggregates_DA(DM dac,DM daf,Mat *rest)
{
  ...
  ierr = ISLocalToGlobalMappingGetSize(ltogmf,&idx_f_len);CHKERRQ(ierr);
  ...
  ierr = ISLocalToGlobalMappingGetSize(ltogmc,&idx_c_len);CHKERRQ(ierr);
  ...
  */
  ...
  max_agg_size = 2*(Mf/Mc+1)*(Nf/Nc+1)*(Pf/Pc+1);
  ...
  /*ierr = MatSetOption(*rest, MAT_NEW_NONZERO_ALLOCATION_ERR, PETSC_FALSE);CHKERRQ(ierr);*/
  ...
  printf("Mf=%i Mc=%i m_f=%i m_c=%i Nf=%i Nc=%i n_f=%i n_c=%i Pf=%i Pc=%i p_f=%i p_c=%i\n",
    Mf,Mc,m_f,m_c,Nf,Nc,n_f,n_c,Pf,Pc,p_f,p_c);
    ...
    for (j_c=j_start_c; j_c<j_start_c+n_c; j_c++) {
      for (i_c=i_start_c; i_c<i_start_c+m_c; i_c++) {
        for (d=0; d<dofc; d++) {
          ...
          */
          /*if (l_c    *Pf/Pc < l_start)     printf("l_c    *Pf/Pc=%i*%i/%i=%i less    than %i\n",l_c    ,Pf,Pc,l_c    *Pf/Pc,l_start    );
          if ((l_c+1)*Pf/Pc > l_start+p_f) printf("(l_c+1)*Pf/Pc=%i*%i/%i=%i greater than %i\n",(l_c+1),Pf,Pc,(l_c+1)*Pf/Pc,l_start+p_f);
          if (j_c    *Nf/Nc < j_start)     printf("l_c    *Pf/Pc=%i*%i/%i=%i less    than %i\n",j_c    ,Nf,Nc,j_c    *Nf/Nc,j_start    );
          if ((j_c+1)*Nf/Nc > j_start+n_f) printf("(l_c+1)*Pf/Pc=%i*%i/%i=%i greater than %i\n",(j_c+1),Nf,Nc,(j_c+1)*Nf/Nc,j_start+n_f);
          if (i_c    *Mf/Mc < i_start)     printf("i_c    *Mf/Mc=%i*%i/%i=%i less    than %i\n",i_c    ,Mf,Mc,i_c    *Mf/Mc,i_start    );
          if ((i_c+1)*Mf/Mc > i_start+m_f) printf("(i_c+1)*Mf/Mc=%i*%i/%i=%i greater than %i\n",(i_c+1),Mf,Mc,(i_c+1)*Mf/Mc,i_start+m_f); */
          /*for (l=PetscMax(l_c*Pf/Pc,l_start); l<PetscMin((l_c+1)*Pf/Pc,l_start+p_f); l++) {
            for (j=PetscMax(j_c*Nf/Nc,j_start); j<PetscMin((j_c+1)*Nf/Nc,j_start+n_f); j++) {
              for (i=PetscMax(i_c*Mf/Mc,i_start); i<PetscMin((i_c+1)*Mf/Mc,i_start+m_f); i++) {*/
          for (l=PetscMax(l_start+(l_c-l_start_c)*p_f/p_c,l_start); l<PetscMin(l_start+(l_c-l_start_c+1)*p_f/p_c,l_start+p_f); l++) {
            for (j=PetscMax(j_start+(j_c-j_start_c)*n_f/n_c,j_start); j<PetscMin(j_start+(j_c-j_start_c+1)*n_f/n_c,j_start+n_f); j++) {
              for (i=PetscMax(i_start+(i_c-i_start_c)*m_f/m_c,i_start); i<PetscMin(i_start+(i_c-i_start_c+1)*m_f/m_c,i_start+m_f); i++) {
                fine_nodes[fn_idx] = idx_f[doff*(m_ghost*n_ghost*(l-l_start_ghost) + m_ghost*(j-j_start_ghost) + (i-i_start_ghost))] + d;
                if (fine_nodes[fn_idx] > Mf*Nf*Pf*doff) {
                  printf("idx=%i i=%i j=%i l=%i d=%i loc=%i glob=%i locmax=%i globmax=%i idxlen=%i\n",
                    fn_idx,i,j,l,d,doff*(m_ghost*n_ghost*(l-l_start_ghost) + m_ghost*(j-j_start_ghost) + (i-i_start_ghost)),
                    fine_nodes[fn_idx],m_f*n_f*p_f*doff,Mf*Nf*Pf*doff,idx_f_len);
                  printf("i_c=%i Mf=%i Mc=%i j_c=%i Nf=%i Nc=%i l_c=%i Pf=%i Pc=%i\n",
                    i_c,Mf,Mc,j_c,Nf,Nc,l_c,Pf,Pc);
                  printf("i=%i i<Min(%i,%i) j=%i j<Min(%i,%i) l=%i l<Min(%i,%i)\n",
                    i_c*Mf/Mc,(i_c+1)*Mf/Mc,Mf,j_c*Nf/Nc,(j_c+1)*Nf/Nc,Nf,l_c*Pf/Pc,(l_c+1)*Pf/Pc,Pf);
                  printf("i_ghost=(%i:%i) m_ghost=%i j_ghost=(%i:%i) n_ghost=%i l_ghost=(%i:%i) p_ghost=%i\n",
                    i_start_ghost,i_start_ghost+m_ghost-1,m_ghost,
                    j_start_ghost,j_start_ghost+n_ghost-1,n_ghost,
                    l_start_ghost,l_start_ghost+p_ghost-1,p_ghost);
                  printf("i=(%i:%i) m=%i j=(%i:%i) n=%i l=(%i:%i) p=%i\n",
                    i_start,i_start+m_f-1,m_f,
                    j_start,j_start+n_f-1,n_f,
                    l_start,l_start+p_f-1,p_f);
                }
                fn_idx++;
              }
            }
          }
          if (fn_idx > max_agg_size) {
            printf("Error: fn_idx=%i > max_agg_size=%i\n",fn_idx,max_agg_size);
          }
